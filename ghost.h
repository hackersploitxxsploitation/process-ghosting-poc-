...
FARPROC DllAddresFuction(LPCSTR Name);//Obtem
//usaremos o endereço para usar as APIS NtReadVitualMemory etc.
void GhostExec(const char* lpszSourceFile);
DWORD GetSizeOfFile(WCHAR *FileName);
//Etapas do Process Ghosting
BOOL ReadRemotePEB(HANDLE proc, PPROCESS_BASIC_INFORMATION pi, OUT PPEB peb_copy);

/* 1- crie um arquivo usando a API NtOpenFile  retorne  o identificar  HANDLE do arquivo7
2-declare as estrutruras   FILE_DISPOSITION_INFORMATION info = { 0 };
    info.DeleteFile = TRUE;  e  coloque o membro info.DeleteFile=True
	DeleteFile

Indica se o arquivo do sistema operacional deve excluir o arquivo quando o arquivo for fechado. Defina este membro como TRUE para excluir o arquivo quando ele for fechado. Caso contrário, defina como FALSE . Definir esse membro como FALSE não terá efeito se o identificador foi aberto com FILE_FLAG_DELETE_ON_CLOSE.
